...
public class UnitTestsStatisticalMetrics
{
    ...
    public void Setup()
    {
        ...
        for (int i = 0; i < _confusionMatrixData.Length; i++)
        {
            _metrics.FillMetric(
                i, 
    ...
    public void FillMetricTest()
    {
        ...
        int[] tp = _metrics.TruePositives;
        int[] tn = _metrics.TrueNegatives;
        int[] fn = _metrics.FalseNegatives;
        int[] fp = _metrics.FalsePositives;
    ...
    public void ComputeMetricsTest()
    {
        ...
        float[] accuracies = _metrics.Accuracies;
        float[] precisions = _metrics.Precisions;
        float[] recalls = _metrics.Recalls;
        float[] f1Scores = _metrics.F1Scores;
        ...
        float[] expF1Scores = [0.833333f, 0.994764f,  0.614035f, 0.964706f, 0.596774f,  1.000000f, 0.621849f, 0.832000f, 0.504854f, 0.532110f];

        for (int i = 0; i < accuracies.Length; i++)
        {
            Console.WriteLine(precisions[i]);
            Assert.LessOrEqual(MathF.Abs(accuracies[i] - expAccuracies[i]), 0.00001f);
            Assert.LessOrEqual(MathF.Abs(precisions[i] - expPrecisions[i]), 0.00001f);
            Assert.LessOrEqual(MathF.Abs(recalls[i] - expRecalls[i]), 0.00001f);
            Assert.LessOrEqual(MathF.Abs(f1Scores[i] - expF1Scores[i]), 0.00001f);
        }
    }
}